...
addon_settings = xbmc.translatePath('special://userdata/addon_data/script.module.openscrapers/settings.xml')

def deleteOSset():

    xbmcgui.Dialog().notification(addon_name, 'Clearing Old Scraper Settings', addon_icon)

    if os.path.exists(xbmc.translatePath(addon_settings)):
        control.deleteFile(addon_settings)

    xbmcgui.Dialog().notification(addon_name, 'Scraper Settings Cleared', addon_icon)
